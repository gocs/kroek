...
// StrokeMap manages the current drag state by mouse.
type StrokeMap struct {
	stroke Stroke
}

// NewStrokeMap generates new struct for the stroke algorithm
func NewStrokeMap(source StrokeSource) *Stroke {
	cx, cy := source.Position()
	return &Stroke{
		source:   source,
		initX:    cx,
...
// Update updates the stroke state
func (sm *StrokeMap) Update() {
	sm.stroke.Update()
}

// IsReleased returns true if stroke is released
func (sm *StrokeMap) IsReleased() bool {
	return sm.stroke.IsReleased()
}

// Position gets the position of the current stroke
func (sm *StrokeMap) Position() (int, int) {
	return sm.stroke.Position()
}

// PositionDiff subtracts the stroke between the previous and current state
func (sm *StrokeMap) PositionDiff() (int, int) {
	return sm.stroke.PositionDiff()
}

// DraggingObject gets anything the stroke drags
func (sm *StrokeMap) DraggingObject() interface{} {
	return sm.stroke.DraggingObject()
}

// SetDraggingObject sets anything the stroke drags
func (sm *StrokeMap) SetDraggingObject(object interface{}) {
	sm.stroke.draggingObject = object
}
